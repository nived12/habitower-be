...
RSpec.describe(ChallengeStepTemplatePolicy, type: :policy) do
  let(:creator) { create(:user) }
  let(:other_user) { create(:user) }
  let(:challenge) { create(:challenge_template, creator: creator) }
  let(:challenge_step) { create(:challenge_step_template, challenge_template: challenge, creator: creator) }
  ...
  describe "#index?" do
    ...
    context "when challenge is private" do
      let(:challenge) { create(:challenge_template, :private, creator: creator) }
  ...
  describe "#create?" do
    context "when user is the challenge creator" do
      it "permits the action" do
        new_step = challenge.challenge_step_templates.build(title: "New Step", position: 1, creator: creator)
    ...
    context "when user is not the challenge creator" do
      it "denies the action" do
        new_step = challenge.challenge_step_templates.build(title: "New Step", position: 1, creator: other_user)
